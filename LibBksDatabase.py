...
def addDataRec(MTy,Ref,Tit,fna,sna,Adr1,Adr2,pcd,MNo,BkID,Bkt,Atr,DBo,Ddu,sPr,LrF,DoD,DonL):
	con=sqlite3.connect("libbooks.db")
	cur=con.cursor()
	cur.execute("INSERT INTO libbooks VALUES (NULL,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(MTy,Ref,Tit,fna,sna,Adr1,Adr2,pcd,MNo,BkID,Bkt,Atr,DBo,Ddu,sPr,LrF,DoD,DonL))
	con.commit()
	con.close()
...
def searchData(MTy="",Ref="",Tit="",fna="",sna="",Adr1="",Adr2="",pcd="",MNo="",BkID="",Bkt="",Atr="",DBo="",\
	Ddu="",sPr="",LrF="",DoD="",DonL=""):
	con=sqlite3.connect("libbooks.db")
	cur=con.cursor()
...
def dataUpdate(id,MTy="",Ref="",Tit="",fna="",sna="",Adr1="",Adr2="",pcd="",MNo="",BkID="",Bkt="",Atr="",DBo="",\
	Ddu="",sPr="",LrF="",DoD="",DonL=""):
	con=sqlite3.connect("libbooks.db")
	cur=con.cursor()
	cur.execute("UPDATE libbooks SET MTY=?, REF=?,TIT=?, FNA=?,SNA=?,ADR1=?,ADR2=?,PCD=?,MNO=?,BKID=?,BKT=?, ATR=?, DBO=?, DDU=?,SPR=?,LRF=?,DOD=?,DONL=? WHERE id=?",(MTy,Ref,Tit,fna,sna,Adr1,Adr2,pcd,MNo,BkID,Bkt,Atr,DBo,Ddu,sPr,LrF,DoD,DonL,id))
	con.commit()
	con.close()
